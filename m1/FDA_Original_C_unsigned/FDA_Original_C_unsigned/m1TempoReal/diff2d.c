...
void diffLUT// diff modificado para chamar a tabela de valores
...
for (i=1; i<=nx; i++)
 for (j=1; j<=ny; j++)

     {
       ...
       /* calculate weights */
       qN = vet[(int)fabs((g[i][j])-(g[i][j+1]))];
       qNE = vet[(int)fabs((g[i][j])-(g[i+1][j+1]))];
       qE = vet[(int)fabs((g[i][j])-(g[i+1][j]))];
       qSE = vet[(int)fabs((g[i][j])-(g[i+1][j-1]))];
       qS = vet[(int)fabs((g[i][j])-(g[i][j-1]))];
       qSW = vet[(int)fabs((g[i][j])-(g[i-1][j-1]))];
       qW = vet[(int)fabs((g[i][j])-(g[i-1][j]))];
       qNW = vet[(int)fabs((g[i][j])-(g[i-1][j+1]))];
